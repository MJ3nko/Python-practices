...
def re_match(domain):
    # Return match object, apply at the start of the string
...
def re_findall(domain):
    capture_domain = r"(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z0-9][a-z0-9-]{0,61}[a-z0-9]"
    results = re.findall(capture_domain, domain)
    if results:
...
def re_search(domain, email, ipv4):
    # Return match object
    results = re.search(r"(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z0-9][a-z0-9-]{0,61}[a-z0-9]", domain)
